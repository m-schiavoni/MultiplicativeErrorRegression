...
plot_multivariate = function(list_in) {
  N = list_in$N
  ...
  for (i in 1:mylen) {
    ...
    b2$LogErr[i] = median(list_in$LogErr$b2[1:ivec[i]], na.rm=TRUE)
    b3$LogErr[i] = median(list_in$LogErr$b3[1:ivec[i]], na.rm=TRUE)
    b0$PING[i] = median(list_in$PING$b0[1:ivec[i]], na.rm=TRUE)
    b1$PING[i] = median(list_in$PING$b1[1:ivec[i]], na.rm=TRUE)
    b2$PING[i] = median(list_in$PING$b2[1:ivec[i]], na.rm=TRUE)
    b3$PING[i] = median(list_in$PING$b3[1:ivec[i]], na.rm=TRUE)
    b0$GRMLN[i] = median(list_in$GRMLN$b0[1:ivec[i]], na.rm=TRUE)
    b1$GRMLN[i] = median(list_in$GRMLN$b1[1:ivec[i]], na.rm=TRUE)
    b2$GRMLN[i] = median(list_in$GRMLN$b2[1:ivec[i]], na.rm=TRUE)
    b3$GRMLN[i] = median(list_in$GRMLN$b3[1:ivec[i]], na.rm=TRUE)
    b0$MUPE[i] = median(list_in$MUPE$b0[1:ivec[i]], na.rm=TRUE)
    b1$MUPE[i] = median(list_in$MUPE$b1[1:ivec[i]], na.rm=TRUE)
    b2$MUPE[i] = median(list_in$MUPE$b2[1:ivec[i]], na.rm=TRUE)
    b3$MUPE[i] = median(list_in$MUPE$b3[1:ivec[i]], na.rm=TRUE)
    b0$ZMPE[i] = median(list_in$ZMPE$b0[1:ivec[i]], na.rm=TRUE)
    b1$ZMPE[i] = median(list_in$ZMPE$b1[1:ivec[i]], na.rm=TRUE)
    b2$ZMPE[i] = median(list_in$ZMPE$b2[1:ivec[i]], na.rm=TRUE)
    b3$ZMPE[i] = median(list_in$ZMPE$b3[1:ivec[i]], na.rm=TRUE)
  }
  ...
  axis(side=1, at=1:mylen, labels=ivec, las=3, cex.axis=0.9)
  mtext(side=2, line=2.5, 'Median of b Estimates', cex=0.9)
  mtext(side=1, line=3, '# Monte Carlo Iterations', cex=0.9)
  ...
  abline(h=list_in$b1, col='red2')
  boxplot(df, col=cols, xlab='', ylab='', xaxt='n', yaxt='n', outline=FALSE, add=TRUE)
  mtext(side=2, line=2.5, 'b Estimates', cex=0.9)
  
  # title and legend
  par(fig=c(0,1,0,1), oma=c(0,0,1,0), mar=c(0,0,0,0), new=TRUE)
  plot(0, 0, type='n', bty='n', xaxt='n', yaxt='n')
  mtext(side=3, line=0, paste0('Monte Carlo Simulation Plots (y = a * x1^b * x2^c * d^x3) [n=', list_in$n, 
                               ';  ', 'cv=', list_in$cv, ';  ', 'err=', list_in$err, ']'), cex=1.0)
  myleg = c('LogErr', 'PING', 'GRMLN', 'MUPE', 'ZMPE')
  ...
  # b2 convergence plot
  ylims = c(min(0.97*list_in$b2, min(b2)), max(1.03*list_in$b2, max(b2)))
  matplot(b2, type='n', ylim=ylims, xaxt='n', xlab='', ylab='', cex.axis=0.9, cex.lab=0.9, new=TRUE)
  rect(par('usr')[1],par('usr')[3],par('usr')[2],par('usr')[4],col='grey90',border=NA); grid(col='white',lty=1); box()
  abline(h=list_in$b2, col='red2')
  matlines(b2, xaxt='n', yaxt='n', col=cols[1:5], lwd=2, lty=c(1,3,1,4,2), new=TRUE)
  axis(side=1, at=1:mylen, labels=ivec, las=3, cex.axis=0.9)
  mtext(side=2, line=2.5, 'Median of c Estimates', cex=0.9)
  mtext(side=1, line=3, '# Monte Carlo Iterations', cex=0.9)
  
  # b2 boxplots
  df = data.frame(LogErr=list_in$LogErr$b2, PING=list_in$PING$b2, GRMLN=list_in$GRMLN$b2, 
                  MUPE=list_in$MUPE$b2, ZMPE=list_in$ZMPE$b2)
  boxplot(df, col=cols, ylab='', las=3, cex.axis=0.9, cex.lab=0.9, outline=FALSE)
  rect(par('usr')[1],par('usr')[3],par('usr')[2],par('usr')[4],col='grey95',border=NA); box()
  abline(h=list_in$b2, col='red2')
  boxplot(df, col=cols, xlab='', ylab='', xaxt='n', yaxt='n', outline=FALSE, add=TRUE)
  ...
  # b3 convergence plot
  ylims = c(min(0.97*list_in$b3, min(b3)), max(1.03*list_in$b3, max(b3)))
  matplot(b3, type='n', ylim=ylims, xaxt='n', xlab='', ylab='', cex.axis=0.9, cex.lab=0.9, new=TRUE)
  rect(par('usr')[1],par('usr')[3],par('usr')[2],par('usr')[4],col='grey90',border=NA); grid(col='white',lty=1); box()
  abline(h=list_in$b3, col='red2')
  matlines(b3, xaxt='n', yaxt='n', col=cols[1:5], lwd=2, lty=c(1,3,1,4,2), new=TRUE)
  axis(side=1, at=1:mylen, labels=ivec, las=3, cex.axis=0.9)
  mtext(side=2, line=2.5, 'Median of d Estimates', cex=0.9)
  mtext(side=1, line=3, '# Monte Carlo Iterations', cex=0.9)
  
  # b3 boxplots
  df = data.frame(LogErr=list_in$LogErr$b3, PING=list_in$PING$b3, GRMLN=list_in$GRMLN$b3, 
                  MUPE=list_in$MUPE$b3, ZMPE=list_in$ZMPE$b3)
  boxplot(df, col=cols, ylab='', las=3, cex.axis=0.9, cex.lab=0.9, outline=FALSE)
  rect(par('usr')[1],par('usr')[3],par('usr')[2],par('usr')[4],col='grey95',border=NA); box()
  abline(h=list_in$b3, col='red2')
  boxplot(df, col=cols, xlab='', ylab='', xaxt='n', yaxt='n', outline=FALSE, add=TRUE)
  mtext(side=2, line=2.5, 'd Estimates', cex=0.9)
