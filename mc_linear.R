...
#
source("./sim_errors.R")
source("./plot_linear.R")
library(minpack.lm)
library(nloptr)
library(stats4)
mc_linear = function(n=15, err='gamma', cv=0.6, N=1, seed=NA) {
  ...
  b_0 = 500;  b_1 = 0.6    # true population parameters
  ...
  Y = (B0 + B1*X) * sim_errors(err, cv, N, n)
  ...
  # define model form #####################################################
  mupe_form = as.formula("y ~ x")  # linear models don't require explicit naming of parameters
  y_hat_func = function(par, x) return(par[1] + par[2]*x)
  ...
  for (i in 1:N) {
    ...
    # use OLS solution as starting guess
    OLS = lm(mupe_form)
    start_coeffs = OLS$coefficients
    y_hat = y_hat_func(start_coeffs, x)
    ...
    if (class(logErr) != 'try-error') {
      if (logErr$info != 0 & logErr$info != 5 & logErr$info != 9) {
        ...
        ping_sol = exp((1 - p/n)*var_hat/2) * logErr$par  # PING factor applied to all parameters in a linear model
    ...
    ## GRMLN ######################################
    grmln = try(suppressWarnings(optim(par=c(start_coeffs, var_hat0), fn=min_grmln, x=x, p=p, 
    ...
    ## MUPE #######################################################
    mupe = OLS;  conv = 1.0;  outer = 0           # initialize model, convergence, and counter 
    while (conv > 1e-5) {
      wt = 1 / mupe$fitted^2                      # calculate weights
      pbeta = mupe$coef                           # solution of prior iteration
      mupe = lm(mupe_form, weights=wt)            # weighted least squares
      beta = mupe$coef                            # solution of current iteration
      conv = max(abs((beta-pbeta)/beta))          # maximum fractional change in any parameter
      outer = outer + 1;  if (outer == 200) break  # force stop, if necessary
    }
	if (outer < 200) {
      my_list$MUPE[i,] = beta
      if (scatter) {
