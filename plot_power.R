...
plot_power = function(list_in) {
  ...
  b0 = data.frame(LogErr=vector('numeric',mylen), PING=vector('numeric',mylen), MRLN=vector('numeric',mylen), 
  ...
  for (i in 1:mylen) {
    ...
    b0$MRLN[i] = median(list_in$MRLN$b0[1:ivec[i]], na.rm=TRUE)
    ...
    b1$MRLN[i] = median(list_in$MRLN$b1[1:ivec[i]], na.rm=TRUE)
  ...
  # b0 boxplots
  df = data.frame(LogErr=list_in$LogErr$b0, PING=list_in$PING$b0, MRLN=list_in$MRLN$b0, 
  ...
  # b1 boxplots
  df = data.frame(LogErr=list_in$LogErr$b1, PING=list_in$PING$b1, MRLN=list_in$MRLN$b1, 
  ...
  mtext(side=3, line=0, paste0('Monte Carlo Simulation Plots (y = a*x^b) [n=', list_in$n, 
                               ';  ', 'cv=', list_in$cv, ';  ', 'err=', list_in$err, ']'), cex=1.0)
  myleg = c('LogErr', 'PING', 'MRLN', 'MUPE', 'ZMPE')
